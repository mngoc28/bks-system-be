...
Quick reference for Laravel Modules + Blade security hardening.

## Laravel Modules notes (repo-specific)

- Validate input via `FormRequest` at route/controller boundaries.
- Enforce authorization with policies/gates and module-level middleware.
- Keep CSRF enabled on web forms and auth middleware on protected routes.
- Dependency audit is PHP-first in this repo (`composer audit`).
...
- [ ] `config/cors.php` chỉ allow đúng domain cần thiết.
- [ ] Không dùng wildcard `*` khi endpoint có credentials.
- [ ] Route nhóm API chỉ expose methods cần thiết.
...
- [ ] Production không trả stack trace (`APP_DEBUG=false`).
- [ ] API lỗi trả message generic + mã lỗi nội bộ (không lộ SQL/path).
- [ ] Log đầy đủ ở server logs, nhưng response cho client tối giản.
...
| 6 | Vulnerable Components | `composer audit`, keep dependencies updated, minimal deps |
