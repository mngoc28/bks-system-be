...
*   **Phiên bản:** 2.1 (Bổ sung Public Booking — phân loại lưu trú & đếm đêm; từ EU/UAT/Domain review 2026-06-11)
...
*   **Traceability:** EU-RAW-004/005 → UAT-ISSUE-004/005 → [`domain_review_public-booking_uat-004-005_2026-06-11.md`](../reports/domain/domain_review_public-booking_uat-004-005_2026-06-11.md)
...
| :--- | :--- | :--- | :--- |
| **REQ-FIN-001** | **Tính toán Tổng giá trị giao dịch (GMV)**<br>$$\text{GMV} = \text{Tổng tiền phòng gốc} + \text{Tổng tiền dịch vụ đặt trước}$$<br>- *Tổng tiền phòng gốc (ngắn hạn, `unit = day`):* Đơn giá/đêm (đã gồm phụ thu lễ/tết/cuối tuần) $\times$ **số đêm lưu trú** — xem **REQ-STAY-002**.<br>- *Tổng tiền phòng gốc (prorate tháng):* Khi chỉ có gói `month` và lưu trú &lt; 30 ngày: $\text{giá tháng} \times (\text{số ngày lịch} / 30)$ — xem **REQ-STAY-003**.<br>- *Tiền dịch vụ:* Các dịch vụ bổ trợ chọn trước (ăn sáng, đưa đón...).<br>- *Mã giảm giá:* GMV tính trước khi trừ mã giảm giá (nếu hệ thống tài trợ) hoặc sau khi trừ (nếu Partner tài trợ). | **Must** | Approved (v2.1 clarified) |
...
| **REQ-DOC-003** | **Copy pháp lý trên Public Booking (bước xác nhận)**<br>- *Ngắn hạn* (`stay_segment = short_term`): Hiển thị *"Bằng việc xác nhận, bạn đồng ý với các Điều khoản & Điều kiện lưu trú ngắn hạn"*. **Cấm** nhắc *"Hợp đồng thuê nhà"* hoặc *"lưu trú dài hạn"*.<br>- *Dài hạn* (`stay_segment = long_term_lease`): Hiển thị *"Sau khi đối tác xác nhận, bạn sẽ được yêu cầu ký Hợp đồng thuê nhà điện tử tại Hồ sơ lưu trú"*.<br>- Phân loại phải khớp **REQ-STAY-001** và kết quả sinh hợp đồng BE (`TERMS_AND_CONDITIONS` vs `LEASE_AGREEMENT`). | **Must** | 🆕 Draft |
...
### 3.7 Phân hệ Phân loại Lưu trú & Public Booking (Stay Classification)

> **Nguồn:** Domain review DOM-PB-001 → 006. Khắc phục UAT-ISSUE-004, UAT-ISSUE-005.

#### 3.7.1 Thuật ngữ (Glossary)

| Thuật ngữ | Định nghĩa |
| :--- | :--- |
| **Đêm (night)** | Đơn vị bán phòng ngắn hạn. Check-in ngày $D_0$, check-out ngày $D_1$: $\text{nights} = \max(1,\ \text{diffCalendarDays}(D_0, D_1))$. Ngày check-out **không** tính thêm một đêm (khớp khoảng lưu trú `[start, end)` — §4.2). |
| **Ngày lịch (calendar day)** | Dùng khi prorate gói tháng: số ngày từ check-in đến check-out **bao gồm** cả hai mốc (inclusive) cho mục đích tính tỷ lệ tháng. |
| **`short_term`** | Voucher (`TERMS_AND_CONDITIONS`), cọc theo REQ-SDEP-001/002. |
| **`long_term_lease`** | Hợp đồng (`LEASE_AGREEMENT`), cọc escrow theo REQ-DEP-001. |

#### 3.7.2 Yêu cầu chức năng

| ID Yêu cầu | Mô tả chi tiết chức năng | Độ ưu tiên | Trạng thái |
| :--- | :--- | :--- | :--- |
| **REQ-STAY-001** | **Phân loại lưu trú thống nhất (`resolveStayClassification`)**<br>Hệ thống **bắt buộc** dùng một hàm/logic chung (FE + BE) với đầu vào: `property_type` (slug hoặc id ánh xạ `property_types`), `selected_price_unit` (`day` \| `month`), `stay_nights`. **Không** dùng magic number `property_type_id` hard-code.<br><br>**Ma trận quyết định:**<br><br>\| Loại hình (`slug`) \| Điều kiện \| `stay_segment` \| Hợp đồng \|<br>\| :--- \| :--- \| :--- \| :--- \|<br>\| `khach-san-hotel`, `nha-nghi-guesthouse` \| Mọi độ dài \| `short_term` \| Voucher \|<br>\| `homestay-co-chia-phong` \| `stay_nights` &lt; 30 \| `short_term` \| Voucher \|<br>\| `homestay-co-chia-phong` \| `stay_nights` ≥ 30 \| `long_term_lease` \| Lease \|<br>\| `can-ho-dich-vu-theo-phong` \| `stay_nights` &lt; 30 **và** gói `day` \| `short_term` \| Voucher \|<br>\| `can-ho-dich-vu-theo-phong` \| `stay_nights` ≥ 30 **hoặ** gói `month` \| `long_term_lease` \| Lease \| | **Must** | 🆕 Draft |
| **REQ-STAY-002** | **Đếm đêm & nhãn UI ngắn hạn**<br>- Công thức: $\text{stay\_nights} = \max(1,\ \text{diffCalendarDays}(\text{check-in},\ \text{check-out}))$.<br>- Ví dụ: 21/06 → 23/06 = **2 đêm**, không phải 3 ngày.<br>- UI Public Booking / Stay Portal: nhãn **"Tổng số đêm: X đêm"**; dòng phí **"X đêm × [đơn giá/đêm]"**.<br>- Tổng phòng (`unit = day`): $\text{unitPrice} \times \text{stay\_nights}$. | **Must** | 🆕 Draft |
| **REQ-STAY-003** | **Prorate gói tháng (&lt; 30 ngày lịch)**<br>- Chỉ áp dụng khi phòng **chỉ có** gói `month` hoặc khách chọn gói `month` với lưu trú &lt; 30 ngày.<br>- $\text{roomTotal} = \text{monthlyPrice} \times (\text{calendarDays} / 30)$ với `calendarDays` = inclusive.<br>- UI: **"X ngày (tính theo gói tháng)"** — không dùng nhãn "đêm". | **Must** | 🆕 Draft |
| **REQ-STAY-004** | **Lý do hiển thị khi yêu cầu cọc (deposit reason labels)**<br>- Ngắn hạn: chỉ các lý do *"Đặt phòng sát giờ (trong vòng 24h)"*, *"Thời gian lưu trú có ngày Cuối tuần"*, *"Mùa cao điểm / Ngày lễ"* (nếu có).<br>- **Cấm** gắn *"Thuê dài hạn (Căn hộ / Dịch vụ dài hạn)"* khi `stay_segment = short_term`.<br>- Dài hạn: được phép dùng lý do *"Thuê dài hạn"* / escrow. | **Must** | 🆕 Draft |
| **REQ-STAY-005** | **Đồng bộ preview ↔ BE ↔ email**<br>Số đêm/ngày, tổng tiền phòng, `stay_segment`, loại hợp đồng dự kiến trên Public Booking **phải khớp** payload tạo booking BE và nội dung email xác nhận. Sai lệch = defect P0. | **Must** | 🆕 Draft |

#### 3.7.3 Acceptance Criteria (Public Booking — UAT retest)

| ID | Given | When | Then |
| :--- | :--- | :--- | :--- |
| **AC-PB-01** | Phòng Nhà nghỉ (`nha-nghi-guesthouse`), 2 đêm | Khách tới bước xác nhận Public Booking | Copy **không** nhắc Hợp đồng thuê nhà; `stay_segment = short_term` |
| **AC-PB-02** | Check-in 21/06, check-out 23/06, gói `day`, giá 740.856/đêm | Xem tổng tạm tính | Hiển thị **2 đêm**; tổng phòng = **1.481.712đ** (2 × 740.856) |
| **AC-PB-03** | Cùng AC-PB-02, có T7/CN trong kỳ lưu trú | Xem lý do cọc | Lý do **"Cuối tuần"** (hoặc tương đương), **không** "Thuê dài hạn" |
| **AC-PB-04** | Căn hộ DV, 2 đêm, gói `day` | Bước xác nhận | Voucher ngắn hạn, không lease |
| **AC-PB-05** | Căn hộ DV, ≥ 30 đêm hoặc gói `month` | Bước xác nhận | Copy lease + cọc dài hạn |
| **AC-PB-06** | Homestay, 29 đêm vs 30 đêm | So sánh 2 booking | 29 = short_term; 30 = long_term_lease |
| **AC-PB-07** | Bất kỳ AC-PB-01 → 06 | Tạo booking + nhận email | Số tiền & phân loại khớp preview |

> **Phạm vi ngoài AC này:** UAT-ISSUE-003 (m² vs mô tả phòng 1247) — xử lý riêng theo data quality listing.
...
*   **Khoảng lưu trú & đếm đêm (v2.1):** Khoảng booking/block dùng `[start_date, end_date)` — ngày `end_date` là ngày trả phòng, **không** chiếm inventory thêm một đêm. Công thức `stay_nights` tại **REQ-STAY-002** phải nhất quán với quy tắc này.

### 4.3 Ánh xạ `property_types` (tham chiếu REQ-STAY-001)

| ID (seed) | Slug | Nhóm nghiệp vụ |
| :---: | :--- | :--- |
| 1 | `khach-san-hotel` | Ngắn hạn — luôn voucher |
| 2 | `nha-nghi-guesthouse` | Ngắn hạn — luôn voucher |
| 3 | `can-ho-dich-vu-theo-phong` | Linh hoạt — theo đêm/gói & ngưỡng 30 đêm |
| 4 | `homestay-co-chia-phong` | Linh hoạt — &lt; 30 đêm voucher; ≥ 30 đêm lease |
...
#### Q5: "21 đến 23 là 2 đêm hay 3 ngày? Sao nhân 3 lần giá?" (EU-RAW-004)
> **A5:** Theo **REQ-STAY-002**, đây là **2 đêm**. Hệ thống cũ dùng ngày lịch inclusive (+1) — **sai** với chuẩn OTA và đã được sửa trong SRS v2.1. Tổng phòng = 2 × đơn giá/đêm.

#### Q6: "Nhà nghỉ cuối tuần sao lại 'thuê dài hạn' và ký hợp đồng thuê nhà?" (EU-RAW-005)
> **A6:** Đây là **lỗi phân loại UI** (hard-code `property_type_id`), không phải chính sách nghiệp vụ. Nhà nghỉ luôn `short_term` — **REQ-STAY-001**, **REQ-DOC-003**. Cọc cuối tuần vẫn có thể 50% theo **REQ-SDEP-001** nhưng lý do phải là *"Cuối tuần"*, không phải *"Thuê dài hạn"* (**REQ-STAY-004**).

#### Q7: "Có cần hiển thị breakdown trẻ em trên search không?" (EU-RAW-007)
> **A7:** **Should** — ngoài phạm vi v2.1 blocker; ghi nhận enhancement riêng (hiển thị "2 NL, 2 trẻ" thay vì "4 Khách").

---
**Chữ ký Duyệt Tài Liệu (Sign-off Signature):**
*   *Senior Business Analyst:* **Đã cập nhật v2.1** (2026-06-11)
*   *Senior Hospitality Domain Expert:* **Đã duyệt** (domain review 2026-06-11)
*   *Technical Lead Architect:* **Chờ duyệt** (triển khai REQ-STAY-001 → 005)
