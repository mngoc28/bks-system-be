...
| Ownership check tập trung | Một policy duy nhất kiểm `properties.user_id = Auth::id()` thay vì rải rác trong từng query. |
...
| `BookingPolicy` | `view`, `confirm`, `cancel`, `no_show`, `update` | `Auth`, `Booking`, `Property` | Kiểm `properties.user_id = Auth::id()` |
| `RoomBlockPolicy` | `view`, `create`, `delete` | `Auth`, `Room`, `Property` | Tương tự |
...
| Ownership Partner | Policy + scope query `whereHas('room.property', fn($q)=>$q->where('user_id', Auth::id()))` |
...
- BE đã có `bookings`, `rooms`, `properties`, `contracts`, `utility_fees` (xem migration 2025/2026 hiện có).
...
| Authorization theo bản ghi | `BookingPolicy` và `RoomBlockPolicy` kiểm `Property::user_id === Auth::id()` qua quan hệ `room.property` |
| Authorization broadcast | Custom `BroadcastAuthController` lấy JWT từ header, parse user, đối chiếu channel name: `private-partner.{partnerId}` chỉ pass nếu `Auth::id() === partnerId`; `private-property.{propertyId}` chỉ pass nếu Partner sở hữu property |
...
| Static (provinces, property types) | Đã cache; không thay đổi | - | - |
...
| Composite index (`properties.user_id`, `rooms.id`) | Có thể giảm JOIN cost nhưng cần đo trước |
