...
#### Delta đề xuất cho `srs_admin_revenue_reconciliation.md`

| Column | Type | Nullable | Key | Notes |
|---|---|---|---|---|
| payment_collected_at | timestamp | Yes | - | Partner xác nhận "đã thu tiền khách tại quầy" |
| settlement_period_id | bigint | Yes | FK, Index | partner_settlement_periods.id | Liên kết kỳ đối soát đang chứa booking này |
...
| region_label | string(255) | - | Khu vực / vùng liên quan (hiển thị) |
| province_id | bigint | FK -> provinces.id | Tỉnh/thành của điểm; bắt buộc khớp `properties.province_id` khi map phòng (PLAN-RTM-GEO-008) |
...
Mục đích: lưu thông tin tổng hợp của một kỳ đối soát của đối tác (Partner nộp phí 5% hoa hồng cho BKS).

| Column | Type | Nullable | Key | Reference | Notes |
|---|---|---|---|---|---|
| id | bigint | No | PK | - | Khóa chính tự tăng |
| partner_id | bigint | No | FK, Index | users.id | ID của đối tác |
| period_start | date | No | - | - | Ngày bắt đầu kỳ đối soát |
| period_end | date | No | - | - | Ngày kết thúc kỳ đối soát |
| issue_date | date | No | - | - | Ngày phát hành bảng kê đối soát |
| total_gmv | decimal(15,2) | No | - | - | Tổng GMV của kỳ |
| total_commission | decimal(15,2) | No | - | - | Tổng hoa hồng cần thu |
| commission_rate | decimal(5,4) | No | - | - | Tỷ lệ hoa hồng áp dụng, mặc định 0.0500 |
| status | varchar(20) | No | Index | - | draft, issued, paid, disputed, closed |
| issued_at | timestamp | Yes | - | - | Mốc thời gian phát hành |
| paid_at | timestamp | Yes | - | - | Mốc đối tác thanh toán |
| payment_reference | varchar(100) | Yes | - | - | Mã giao dịch chuyển khoản |
| confirmed_by | bigint | Yes | FK | users.id | Admin xác nhận thanh toán |
| note | text | Yes | - | - | Ghi chú điều chỉnh |
| created_at / updated_at | timestamp | No | - | - | Laravel timestamps |

**Unique constraint:** `(partner_id, period_start, period_end)`.

### settlement_line_items

Mục đích: lưu chi tiết các đơn đặt phòng thuộc kỳ đối soát.

| Column | Type | Nullable | Key | Reference | Notes |
|---|---|---|---|---|---|
| id | bigint | No | PK | - | Khóa chính tự tăng |
| settlement_period_id | bigint | No | FK, Index | partner_settlement_periods.id | Liên kết kỳ đối soát (ON DELETE CASCADE) |
| booking_id | bigint | No | FK, Index | bookings.id | Liên kết booking |
| booking_code | varchar(32) | No | - | - | Denormalize mã booking |
| checkout_date | date | No | - | - | Ngày check-out thực tế của khách |
| room_gmv | decimal(15,2) | No | - | - | Tiền phòng của booking tại thời điểm chốt |
| services_gmv | decimal(15,2) | No | - | - | Tiền dịch vụ của booking tại thời điểm chốt |
| total_gmv | decimal(15,2) | No | - | - | Tổng GMV booking |
| commission_amount | decimal(15,2) | No | - | - | Tiền hoa hồng booking |
| snapshot_status | tinyint | No | - | - | Trạng thái booking tại thời điểm đối soát (mặc định 3) |
| created_at / updated_at | timestamp | No | - | - | Laravel timestamps |
...
11. Bảng kê đối soát (`partner_settlement_periods`) được quét định kỳ từ booking `COMPLETED` + `checked_out`. Khi booking được chốt kỳ, cột `bookings.settlement_period_id` phải gán `partner_settlement_periods.id` để khóa dữ liệu và tránh double-count.
